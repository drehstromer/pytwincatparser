...
def get_var(content):
    ...
    def is_variable_declaration(line):
        ...
        return (re.match(r'^[_A-Za-z][_A-Za-z0-9]*\s+.*?;', line) or 
                re.match(r'^[_A-Za-z][_A-Za-z0-9]*,.*?;', line) or 
                re.match(r'^\s*[_A-Za-z][_A-Za-z0-9]*,.*?;', line))

    def is_start_of_new_declaration(line):
        """Check if a line is the start of a new declaration."""
        stripped_line = line.strip()
        return (not stripped_line or 
                is_standalone_line_comment(stripped_line) or 
                is_standalone_block_comment(stripped_line) or 
                is_attribute_declaration(stripped_line) or 
                is_variable_declaration(stripped_line))
    ...
    def process_attribute_declaration(lines, start_index):
        ...
        attribute_lines = [lines[start_index].strip()]  # Use the stripped line without whitespace
        ...
        while current_index < len(lines):
            ...
            if is_variable_declaration(stripped_line):
                ...
                while (next_index < len(lines) and 
                    not is_start_of_new_declaration(lines[next_index]) and 
    ...
    def process_variable_declaration(lines, start_index):
        ...
        while (current_index < len(lines) and 
            not is_start_of_new_declaration(lines[current_index]) and 
    ...
    content_without_keywords = re.sub(r'^\s*(PERSISTENT|CONSTANT)\b', '', content, flags=re.IGNORECASE)
    ...
    while line_index < len(lines):
        ...
        if is_standalone_line_comment(stripped_line):
            ...
            line_index, attribute_var_decl = process_attribute_declaration(lines, line_index)
...
def get_comment_content(decl):
    ...
    for match in block_comment_matches:
        ...
        if full_comment.startswith("(* "):
            ...
            doc_match = re.match(doc_keyword_pattern, comment_content.strip(), re.DOTALL)
